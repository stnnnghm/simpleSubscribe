...
func deleteEmailFromDynamoDB(email, id string) (*dynamodb.DeleteItemOutput, error) {
	table := os.Getenv("DB_TABLE_NAME")
	svc := dynamodb.New(session.New())
...
// Send a confirmation email with a a link to complete subscription
func sendEmailWithSES(email, id string) (*ses.SendEmailOutput, error) {
	svc := ses.New(session.New())
	log.Print("EMAIL: ", email)

	// HTML Format
	msg := fmt.Sprintf("<p>Hello! You're receiving this email because you requested a subscription to our list.</p><p>To complete your subscription, please click this link to finish signing up:</p><p><a class=\"ulink\" href=\"%s%s/?email=%s&id=%s\" target=\"_blank\">Confirm subscription</a>.</p><p>If you did not request this email, you can safely ignore it. Your email address has not yet been added to my list.</p>", os.Getenv("API_URL"), os.Getenv("VERIFY_PATH"), email, id)
...
func lambdaHandler(ctx context.Context, event events.APIGatewayV2HTTPRequest) (events.APIGatewayV2HTTPResponse, error) {
	...
	if event.RawPath == fmt.Sprintf("/%s/", os.Getenv("SUBSCRIBE_PATH")) {
		...
		if uerr != nil {
			log.Print("Could not update db: ", uerr)
			resp.Headers["Location"] = errorPage
			return resp, uerr
		}
